...
USGS.flow.data <- read.csv("/Users/cristiana/Documents/Duke/DataAnalytics/Environmental_Data_Analytics_2020/Data/Raw/USGS_Site02085000_Flow_Raw.csv")
...
# read.csv(file.choose()) # should comment which file you're actually choosing since this doesn't show path
...
> ANSWER: the readme file enables someone else to back and check the source - e.i., download the same exact file the originator downloaded - which could be useful for troubleshooting. 
...
# colnames() is a function - just col names spits out what the col names are
# using assignment operator allows you to rename the columns

# Rename columns -- have to list every column name when doing this - can do specific col names using square brackets (e.g., colnames(USGS.flow.data)[0] <- c('col name'))
...
str(USGS.flow.data) # str means structure, not string
dim(USGS.flow.data) 
length(USGS.flow.data) # gives number of columns
nrow(USGS.flow.data) # gives number of rows

head(USGS.flow.data) # gives first 6 rows
head(USGS.flow.data, 10) # gives first 10 rows
tail(USGS.flow.data, 5) # gives last 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #matrix subsetting, outputting specific pieces
...
> Answer: if you have presence/absence data for an animal and only want to run an analysis on rows that have the presence of data
# also it seems like the R function 'mean' doesn't take the mean of the numbers in a field when NA's are present, 
# it was only after I removed NA's from the discharge.mean field that R was able to calculate a numeric mean
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") # telling R what date format our USGS data is in

USGS.flow.data$datetime # now R spits out that column in the yyyymmdd format, however the legacy data are a problem (post 1970 it spits out 2069 instead of 1969, etc.)
```
...
```{r}
# what is the difference between above ...as.Date() and just below this ...format() ?
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #reformatted date , careful of legacy data (prior to 1970)
...
## ** come back and understand the fucntion that dictated earlier dates
```
...
```{r}
# first I created the subfolder, 'processed' , then ran the next line successfully (wrote file and saved it to this folder)
write.csv(USGS.flow.data, file = "./Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE)

```

# my homework:
  # play with datetime function
  # practice with my own csv
