...
# 3. Import datasets
USGS.flow.data <- read.csv("/Users/cristiana/Documents/Duke/DataAnalytics/Environmental_Data_Analytics_2020/Data/Processed/USGS_Site02085000_Flow_Processed_Sena.csv")
...
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) + # aes = aesthetics, seems to dictate the axes
  geom_bar()
```
...
# make bins (or bars) that are only 10 units wide
ggplot(USGS.flow.data) +
...
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  ...
  scale_x_continuous(limits = c(0, 500)) # only shows 0 - 500 on x axis
...
# just running code will have the graph show up in lower right (under plots)
# if we name the plot something, it won't automatically display but it will be saved in environments
  # to save env and visualize you can save and then print
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "maroon") + # chances color of line
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2, color = 'darkgrey') + # (line types) lty 2 = dashed line, 3 = dotted, etc.
  scale_x_continuous(limits = c(0, 10)) # adjusts scale of x axis
...
#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) + # separate color based on approval code
...
# dif distributions across a range of data (certain sections of datasets have dif dist than others)
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# can show info about sample counts a bit better (wider circle area = more data available)
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75)), # don't know why my comma didn't work..
              scale = "count")
```
...
ggplot(USGS.flow.data) +
  geom_point(aes(x = datetime, y = discharge.mean)) # nice visualization of timescale of dataset
```
