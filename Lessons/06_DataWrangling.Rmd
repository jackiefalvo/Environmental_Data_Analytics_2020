...
> Add notes here: Looking at long term trends of these factors which can influence the health of the species in the lake and the ecosystem as a whole. Being able to look at these long term data can also be translated to policies and regulatory structures.
...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y")
class(NTL.phys.data$sampledate)
# then how to reformat? I forget
# also what distinguishes between a number type and a factor type
```
...
```{r, results = "hide"}
vignette("dplyr") # shows the documentation for dplyr
# does everything have a vignette? do things have a vig and a help page? whats the difference?
```
...
`==` # equal to (can be used with factors or numeric values)
`!=` # not equal to
`<` # less than
...
class(NTL.phys.data$depth) # depth of the lake in meters

head(NTL.phys.data)
view(NTL.phys.data$depth)

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # matrix notation, row, col -- leaving col blank
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
NTL.phys.data.surface1

# dplyr filtering
NTL.phys.data.surface2 <- dplyr::filter(NTL.phys.data, depth == 0)
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, NTL.phys.data$lakename == "Paul Lake" | NTL.phys.data$lakename == "Peter Lake")
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, NTL.phys.data$lakename %in% c("Paul Lake", "Peter Lake")) 
# %in% = 'includes' 
# line above means that we're looking for rows where lake name includes: (list of names - in this case paul, peter)
...
head(NTL.phys.data)
range1 <- filter(NTL.phys.data, NTL.phys.data$year4 >= 1990 & NTL.phys.data$year4 <= 1999) 
view(range1)
range1
```
Question: Why don't we filter using row numbers?

> Answer: data row positions could change through processing, cleaning, etc.
...
```{r}
# pipe operator = %>% pipe means 'then'
  # first take this dataset, then
  # filter for paul lake and peter lake, then
  # select columns, then
  # change date to celcius
# this one 'line' spits out everything

## * pipes and 'ifelse' statements can both work in most circumstances
# use is based on personal preference mostly
# although there can be certain situations where one is more appropriate than the other
...
# this writes and saves the changes we made to a new file
  # * hmm I think for my IHH I'm going to need to 'write' the data a different way to rearrange
# rownames = false means don't rename rows with numbers (default, not a big deal)
```
