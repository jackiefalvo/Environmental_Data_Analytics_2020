...
print(O3plot) # using print will make the plot consistently print/show in pdf when knitted

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") 
print(O3plot2)
## when color is inside aes() it is specifying another object...
## when color is outside aes() it is saying make the geom_points blue
...
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) + # factor, to group by year 
  geom_point() +
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # allows us to facet across two different variables
print(PMplot.faceted2)

# Plot true time series with geom_line
view(EPAair)
unique(EPAair$Site.Name)
...
PeterPaul_scatter_temp <-
  ggplot(subset(PeterPaul.chem.nutrients, depth == 0)) + # can put aesthetics in ggplot or geompoint
  geom_point(aes(x = daynum, y = temperature_C, color = year4)) + # color inside aes()
  facet_grid(vars(lakename)) # splits into two charts, one for paul one for peter lake
print(PeterPaul_scatter_temp)

## facet_grid splits like so
## facet_grid splits also, but we can specify how many rows to make
...
# Adjust axes
## slim and ylim: zooms in on x and y axes where most of data is
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm, se = FALSE) + # line of best fit, default method.. method = lm = linear mode?
  # confidence interval of model, not data ..?; can get rid of with se = false
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? # fill covers up box plots on bottom
print(Nutrientplot3)
...
print(Nutrientplot4)
## every data record is graphed as a point
## points of the same value are lined up next to each other, making area look wider on graph
...
print(Nutrientplot5)
## also gives a wider or skinnier look based on frequency
