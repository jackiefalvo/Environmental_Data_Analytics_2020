...
date: "Spring 2020 - thursday, feb 6"
output: pdf_document
...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
    ...
    ## position = top, right, left, bottom; justification = 1:1
...
theme_set(mytheme) # sets theme at start of session, all plots default to this theme
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
## note that ggplot layers things in order of how they're listed in code
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # dashed horiz line at 50
  geom_hline(yintercept = 100, lty = 2) + # dashed horiz line at 100
  geom_point(alpha = 0.5, size = 1.5) + # points on plot
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + # labels
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), # dictates date
    date_breaks = "2 months", date_labels = "%b %y") + # %b = month abbr %B = full month
  ylab(expression("O"[3]* " AQI Value")) + # "[]*" = subscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) # x axis text adjustments
print(O3plot7)  

## the point alpha .5 setting makes points 50% transparent, to me the points look like two different 
## types (grey and black) the actual meaning is grey is single point in its own space
## and when points overlap they appear darker
```
...
# peter and paul lakes, concentrations of 
NvsP <-
...
NvsP2 <-
  ...
  #xlab("tp_ug (micrograms per L)") +
  #ylab("tn_ug (micrograms per L") +
  labs(x = "tp_un (ml)", 
       y = "tn_ug (ml)", 
       color = "Depth (m)") + # change your legend labels here
  scale_shape_manual(values = c(15, 17)) +
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1, end = .89) +
...
# place your additional edits here
  labs(x = "", y = "Concentration (mg/L)", color = "Nutrients") + 
  ## each argument in labs() relates to arguments in ggplot() ??
