...
The one-sample test (model of the mean), two-sample t-test, analysis of variance (ANOVA), and linear regression are all special cases of the **generalized linear model** (GLM). The GLM also includes analyses not covered in this class, including logistic regression, multinomial regression, chi square, and log-linear models. 

The common characteristic of general linear models is the expression of a continuous response variable as a linear combination of the effects of categorical or continuous explanatory variables, plus an error term that expresses the random error associated with the coefficients of all explanatory variables. The explanatory variables comprise the deterministic component of the model, and the error term comprises the stochastic component of the model. 
...
EPAair.subsample <- sample_n(EPAair, 5000) #takes random assortment of 5000 samples

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone))
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) # ;= continue line, run both lines at once

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") 
# mu = mean, 
# alternative of 'less' means alternative hyp is mean is less than 50
  # default assumes you are assuming greater than and less than 
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(stat = "count", fill = "gray") + # shows the actual count (more jagged/exact)
  #geom_density(fill = "gray") +  # shows statistical dist of density (a bit smoother) 
    # (basically a moving average of the distribution)
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #starts graph at 0
print(Ozone.plot)
```
...
> The plot shows us that our data is concentrated in the space less than 50 (the null hypothesis mean), in other words the data is skewed to the left. 

> KS: EPA ozone measurements for 2018-19 were significantly lower than 50, the AQI threshold for 'good' air quality (one sample t-test; t = -57.98, p < .0001). 

# when p is between .001 and something greater, you'd want to specify the actual value^
...
# showing that our values are significantly different from one another

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) + # we're comparing year, we want factor (otherwise it would treat year as a number / continuous variable)
  geom_freqpoly()

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
O3.twosample
O3.twosample$p.value # can grab sp elements from tests sice they're stored as lists
...
# somehow she took the difference between 2018 mean and 2019 mean
```
...
```{r}
# 1. 
Ozone2.plot <- ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  #scale_alpha_manual(values = c(1, 2) + scale_size_manual(values = c(1, 4))) +
  scale_color_manual(values = c("yellow", "pink"), alpha = c(2, 4)) +
  geom_density(fill = "light blue") +
  geom_vline(xintercept = 50, color = "orange", lty = 2)
print(Ozone2.plot)


Ozone.plot <- ggplot(EPAair, aes(x = Ozone, color = as.factor(Year), size = .5)) +
  geom_density(stat = "count", fill = "gray") + # shows the actual count (more jagged/exact)
  #geom_density(fill = "gray") +  # shows statistical dist of density (a bit smoother) 
    # (basically a moving average of the distribution)
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
