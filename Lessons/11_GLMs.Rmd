...
Litter.Totals <- Litter %>%
  group_by(plotID) %>%
  summarise(dryMass = sum(dryMass)) # our new col for drymass = sum of drymasses for each group's row
## .. I guess I expect to see multiple sum columns (sum for each group) but there is one

# Test for normality.  (see if data is normally distributed)
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  ...
  ylim(0, 85) # sets y axis so top part of letters aren't cut off
print(Litter.Totals.plot)
```

What information does the output give us? How might we report this information in a report?

> The plot shows us the among of dry mass found in various plots along Niwot ridge. 
# I can see that they are different .. but what's the main thing that shows they're significantly different?
...
```{r}
# Format as aov
## shows us that both funct group and land use class are sig predictors
Litter.anova.2way <- aov(data = Litter, dryMass ~ functionalGroup + nlcdClass)
summary(Litter.anova.2way)

# Format as lm
## we can see actual effects of each of them (lm shows a bit more info)
## for anova we're estimating mean of each group, lm output gives you #s you need to est mean
