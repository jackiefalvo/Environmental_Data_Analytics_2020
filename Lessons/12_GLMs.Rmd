...
> Depth accounts for 31% of variance in lake irradiance. 
  > (linear regression, R2 = .31, df = 15,449, p < .0001)
...
## if data points are pretty normal, should fall along horz line
...
par(mfrow = c(2,2), mar=c(1,1,1,1)) # plots them all at once
plot(irradiance.regression)
par(mfrow = c(1,1)) # sets plot window back to 1:1 (one plot at a time)
# this is a verification of the assumptions of our test (shouldn't end up in report)
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + # doesn't change data, just adjusts display
  geom_point() 
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #plots 2 different vars against each other,makes line of best fit
  scale_y_log10() +
  geom_point() 
...
irradiancebydepth2 <- 
    ...
    scale_y_log10() +
    labs(x = "Depth (m)", y = "Irradiance (rz)") +
    geom_smooth(method = 'lm', se = FALSE, color = "black")
print(irradiancebydepth2)
# shows standard error of model, NOT confidence interval of your data
...
# this is good alternative if you're not fulfilling the assumptions of a linear regression
...
# consider variables all at once
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() # omits any row that contains one NA
PeterPaulCorr <- cor(PeterPaulnutrients) # running cor function on this max turns it into a correlation matrix
corrplot(PeterPaulCorr, method = "ellipse")
# stronger correlations = skinnier ellipse, more intense colors
# duplicate values are shown as lines
...
# gives combos (all vars, each var individually)
## computes AIC value for each of the different options
### stacks AIC values and determines which is lowest
### runs the lowest one, with all 4 of the vars, then sees out of new combos which is lowest

# basically compares higherst Rsq you can get and degrees of freedom...
## want a better fit but it's giving us a penalty for adding additional variables
summary(TPmodel)
