...
* **Heteroscedasticity**: The variance of the time series is not constant over time - either gets more variable or less variable over time
...
# although we already specified as.date, filter function wants us to specify date format
...
# [[]] allows us to select 1 col, time series doesn't like the $, so we use brackets
## ts shows up as value -- important to know what data looks like before, since we can't see ts
```
...
# stl is applying a moving avg across data
## stl requires a ts input
...
# We can extract the components (from ts) and turn them into data frames
...
> aggregation: combining two datasets ; bundling data and then scaling up -- combining monthly data and get yearly data, or daily/hourly to monthly, etc.

> subsampling: if you have a lot of data and take out a subset to serve as a representative set of that info; perhaps taking a subset of your longterm data (the cream of the crop); 

> interpolation: filling in gaps between; predicting what would be in your data gaps by extrapolating from existing data
  ->
...
view(EnoDischarge.late.monthly)
...
#starting 10-1-1985
  # we need to dictate this time info because time series is just a list of observations

# Run SMK test
EnoDischarge.late.trend <- smk.test(EnoDischarge.late.monthly.ts)
# smk test
  # is there a trend mono over time
  # then itll tell you if there's a trend for seasons
# null hyp = no monotonic trend ot
  # high p value, means there is no sig trend
  # low z values (close to zero) means can't discern a sig trend
# results
  # the test uses S, different between ts and data..
...
> There is a good deal of variability in our dataset and we did not find a significant mono () trend over time (between 1985 and 2020) or within the months in this time span. 
