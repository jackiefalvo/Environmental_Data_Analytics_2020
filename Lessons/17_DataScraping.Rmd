...
* Manual copy-paste # good for small amounts
* API (retrieve data from standard code; data must be in prescribed format) 
# facebook and google have set ways that you can download data..api..
* DOM parsing # selector gadget
...
# this code does the following:
  # names a column of the data on the webpage something descriptive (we're grabbing one column at a time)
  # directs to webpage (we dictated the url above)
  # dictates which html node we want to grab (we copied and pasted via chrome selector gadget)
  # then reads the html text
    ## this stores the html text in each column as a separate 'value' in R, below we'll transform these into one data frame
...
# total maximum daily load (TMDL) = cap set on amount of contaminant that's allowed into watershed
