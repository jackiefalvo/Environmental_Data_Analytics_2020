...
here("Lessons", "sf-lesson-20200303", "sf_cheetsheet.pdf")
here("Lessons")
here("sf_cheetsheet.pdf")
...
class(gages_raw)
class(southeast_state_bounds_raw)
class(my_tabular_data_raw)
...
While not in base, you can also use glimpse() from the tibble (tidyverse) package to look at basin_nf_seplains_raw. # what's the difference between this and str() ?
...
# questions:
  # what exactly are ESPG's and proj4 strings?
  # if 'setting' = "labeling the data with a CRS it 'already has' " ..
      # so is that like, I projected this shp in arc with wgs84, i now bring it into R and it isn't showing me the CRS...so I'm telling R what the projection is because I know it
      # ..what if you don't know what the projection is?
      # or what if you just make up a csv with random lat/longs that you type in..would you 'set' or 'project' this file to establish a new CRS?
      # whaat happens if..
          # you have data that's already got a set projection; then you do 'set' to new one..
...
# add your code here
gages_na_albers_proj4 <- gages
st_crs(gages_na_albers_proj4) <- my_proj4

gages_na_albers_epsg <- gages
st_crs(gages_na_albers_epsg) <- my_epsg
# set the proj4 string first and then set the EPSG code (because proj4 will over-ride EPSG if you set EPSG first)

st_crs(gages_na_albers_proj4)
st_crs(gages_na_albers_epsg)
# check CRSs ?: why do it separately? the proj4 one says 'no epsg defined', the epsg seems to have everything .. I'll call the epsg one just the normal albers version

gages_na_albers <- gages_na_albers_epsg
